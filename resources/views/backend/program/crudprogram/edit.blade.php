            ...
            <div class="card shadow-lg border-0 rounded-4 p-5">
                <form action="{{ route('programs.update', $program) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_program" class="form-control shadow-sm" value="{{ $program->nama_program }}" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-semibold">Deskripsi</label>
                        <textarea name="deskripsi" class="form-control shadow-sm" rows="5" required>{{ $program->deskripsi }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-semibold">Gambar</label>
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $program->gambar) }}" class="rounded-3 border shadow-sm" width="120">
                        </div>
