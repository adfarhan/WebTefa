    ...
    <main id="main" class="main">
        <div class="container">
            ...
            <div class="card shadow-lg border-0 rounded-4 p-5">
                <form action="{{ route('misi.update', $misi) }}" method="POST">
                    @csrf @method('PUT')

                    <div class="mb-4">
                        <label class="form-label fw-semibold">Misi</label>
                        <textarea name="misi" class="form-control shadow-sm" rows="3" required>{{ $misi->misi }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-semibold">Pilih Visi</label>
                        <select name="visi_id" class="form-control shadow-sm" required>
                            <option value="">-- Pilih Visi --</option>
                            @foreach ($visi as $v)
                                <option value="{{ $v->id }}" {{ $misi->visi_id == $v->id ? 'selected' : '' }}>
                                    {{ $v->visi }}
                                </option>
                            @endforeach
                        </select>
